        ...
        $.get("{{ url('admin/comment')}}"+'/' + value_id +'/edit', function (data) {
           ...
           $('#valueCrudModal').html("ویرایش  کامنت ");
            ...
            if(data.select.state){
                 $("#product_id >[value="+data.select.id+"]").attr('selected',true);
            }else{
                $("#product_id option:selected").removeAttr("selected");
            }
          ...
          if(data.arr.status){
            ...
            jQuery('.alert-danger').append('<p>'+data.arr.msg+'</p>');
              jQuery.each(data.errors, function(key, value){
                ...
                var value = '<tr id="value_id_' + data.comment.id + '"><td>' + data.comment.id + '</td><td> <a  href="/admin/comment/' + data.comment.id + '">' + data.comment.comment + '</a></td><td>'+ data.product.name + '</td><td>'+ data.user.name + '</td>';                                                      
